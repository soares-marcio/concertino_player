...
CREATE TABLE `track` (
  ...
  `apple_trackid` varchar(256) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
ALTER TABLE `recording`
  ADD PRIMARY KEY (`work_id`,`apple_albumid`,`subset`) USING BTREE,
...
ALTER TABLE `recording_performer`
  ...
  ADD KEY `spotify_albumid` (`apple_albumid`,`work_id`,`subset`);

ALTER TABLE `shortrec`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `album` (`work_id`,`apple_albumid`,`subset`);

ALTER TABLE `track`
  ADD PRIMARY KEY (`apple_trackid`) USING BTREE,
  ADD KEY `position` (`position`),
  ADD KEY `cd` (`cd`),
  ADD KEY `work_id` (`work_id`,`apple_albumid`,`subset`);
